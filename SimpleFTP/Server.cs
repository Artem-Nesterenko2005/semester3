...
namespace SimpleFTP
{
    ...
    public class Server
    {
        ...
        private async Task List(string path, StreamWriter writer)
        {
            ...
            var fileSystemEntries = info.GetFileSystemInfos();
            await writer.WriteAsync($"{fileSystemEntries.Length}");

            foreach (var entry in fileSystemEntries)
            {
                await writer.WriteAsync($" {entry} {Directory.Exists(entry.FullName)}");
            }
